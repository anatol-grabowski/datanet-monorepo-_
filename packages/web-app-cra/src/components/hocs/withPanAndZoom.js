...
export default function withPanAndZoom(Child) {
  return class PanAndZoom extends React.Component {
    ...
    move(clientX, clientY) {
      const x =
        this.state.translating.initialWorld[0] + clientX - this.state.translating.initialCursor[0]
      const y =
        this.state.translating.initialWorld[1] + clientY - this.state.translating.initialCursor[1]
    ...
    scale(newScale, centerX, centerY) {
      const rect = this.graphRef.current.getBoundingClientRect()
      const p = [centerX - rect.left, centerY - rect.top]
    ...
    render() {
      ...
      return (
        <div
          className={styles.WithPanAndZoom}
          ref={this.graphRef}
        ...
        >
          <div
            className={styles.TransformedContentWrapper}
            style={{
