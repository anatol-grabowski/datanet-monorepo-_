...
export default class EditArea extends React.Component {
  ...
  render() {
    const { id, coords, text, attachments } = this.props
    const displayedCoords = coords.map((c) => c.toFixed(1)).join(' ')
    const { handleTextChange, handleRemove, handleRemoveAttachment, handleUploadAttachements } =
      this
    return (
      <div className={styles.EditArea}>
        <div>{`node id: ${id}`}</div>
        <br />
        <div>{`node xy: ${displayedCoords}`}</div>
        <br />
        <div>Text:</div>
        <textarea
